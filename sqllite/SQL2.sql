...
--------------------------------------------------------------------------------------------------------------------
  SELECT SUM(accu_realize) as realize, SUM(unrealize) as unrealize FROM (
	  select a.Fund_Code, outstanding.os, MAX(outstanding.seq), outstanding.accu_realize, outstanding.unrealize
	  from transx as a 
